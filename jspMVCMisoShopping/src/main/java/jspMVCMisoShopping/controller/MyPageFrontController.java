...
import jspMVCMisoShopping.service.employee.EmployeePasswordService;
...
public class MyPageFrontController extends HttpServlet {
	protected void doProcess(HttpServletRequest request, 
		...
		if(command.equals("/memberMyPage.my")) {
			...
			EmployeeDetailService action = new EmployeeDetailService();
			action.execute(request);
			RequestDispatcher dispatcher = request.getRequestDispatcher("/myPage/employeeModify.jsp");
			dispatcher.forward(request, response);
			...
			RequestDispatcher dispatcher = 
					request.getRequestDispatcher("myPage/employeePwCon.jsp");
			dispatcher.forward(request, response);
		}else if(command.equals("/employeePwUpdate.my")) {
			HttpSession session = request.getSession();
			AuthInfoDTO auth = (AuthInfoDTO)session.getAttribute("auth");
			String path = null;
			if(request.getParameter("employeePw").equals(auth.getUserPw())) {
				path = "myPage/employeeNewPw.jsp";
			}else {
				request.setAttribute("errPw", "비밀번호가 틀렸습니다.");
				path = "myPage/employeePwCon.jsp";
			}
			RequestDispatcher dispatcher = 
					request.getRequestDispatcher(path);
			dispatcher.forward(request, response);
